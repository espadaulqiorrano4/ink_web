<template>
  <div>
    invoice No.:<b-form-select v-model="invoice_select" signle-line label="Filter">
      <b-form-select-option v-for="invoiceNo in invoiceNos" :key="invoiceNo.invoicenumber_id" :value="invoiceNo.invoicenumber_id">{{invoiceNo.invoiceNo}}</b-form-select-option>
    </b-form-select>
...
export default {
  ...
  methods:{
    async getData() {
          ...
          this.invoiceNos = invoice.data.data
          console.log("data customers:" ,this.invoiceNos)
    ...
    async addbill(){
      let res = await this.$http.post('/bills/create', {
                ...
                invoicenumber_id:this.invoice_select,
